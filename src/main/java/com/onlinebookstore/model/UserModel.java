...
public class UserModel {
	...
	public static ArrayList purchesedBookList() {
		...
		queryString = "SELECT DISTINCT email,name FROM userbuydata where email=?";
	...
	public static boolean purchesedBookData(Book book, UserEmail email) {
		String message = "Thanks for Having Interest in our Store You have successfully purchased [ "
				+ book.getBookName() + "  ] Book" + "   Whose Price is    [ " + book.getBookPrice() + " ]";

		queryString = "Insert into userbuydata values(?,?,?)";
		try {
			con = DatabaseConnectivity.dbConnection();
			preparedStatement = con.prepareStatement(queryString);
